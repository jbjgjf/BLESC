...
- 専用Vercel `blesc-pilot`（`prj_IjgTAtHZ2tKIZ0AWybYJEcPdrm27`）を作成し、保護を維持してデプロイ。URL: https://blesc-pilot.vercel.app、最終確認READY deployment `dpl_AqH5vJpBvsUDZ54u6JHKwLRBzz2G`、アプリSHA `81a8b34`（#190 + #191）。既存本番は変更していない。
...
- #116で未知の認証プロバイダエラーが英語のままUIへ流れる経路を修正。既知・未知のエラーを日本語カタログへ限定し、回帰検査を追加。全画面・全状態・生成応答の日本語受入完了とは扱わない。
- 追加PR #191（#190にstack）でNext.js / eslint-config-nextを16.3.5へ更新し、互換範囲のtransitive dependenciesを修正。分離checkout `81a8b34` でnpm audit 0件（devを含む）、lint、466/466テスト、本番build成功。初回buildは公開Supabase設定不足で失敗したため、CI用placeholder設定を与えて再検証した。鍵・実参加者データは使用していない。

**残る開始条件**: その他の正式決定・研究倫理/学校/データ管理承認、危機連絡先と当番、担当アカウント、purge監視、backup/restore・鍵ローテーション・外部通信監査、成人スタッフの実時間3日演習、3名の最終Go署名。PR #190/#191は人間review/merge待ち。
...
1. C1のメール決定以外の13件、およびC1の受付時間・受諾・学校確認と3役割の正式承認（保持期間・保護者確認・撤回削除窓口・危機対応の当番を含む）。
2. 上記の専用環境に対する運営担当の受諾と担当アカウント・連絡系統。秘密値は共有記録へ貼らない。
