...
create policy "conversation_recall_summaries_own_all" on public.conversation_recall_summaries for all to authenticated using ((select auth.uid()) = owner_user_id) with check ((select auth.uid()) = owner_user_id);
...
begin
  ...
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'conversation_recall_summaries'
  ) then
