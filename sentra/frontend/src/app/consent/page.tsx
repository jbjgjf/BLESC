 ...
 *   - いつでも撤回できる。撤回したとき、保存済みの本文を「いま消す」か
 *     「保存期間まで残す」かは**本人が選ぶ**（#224）。以前はボタン1つで削除が
 *     確定していた。取り消せない選択を1クリックの裏に置かない。
 *     残す方を選んでも撤回は撤回で、研究解析にも学習にも使われない。
 */
  ...
  return (
    <div className="bl-wrap bl-stack">
      ...
      <section className="bl-card bl-stack">
        ...
        {/*
          * 危険の合図が出たときに何が起きるかは、同意の前に伝えないと意味がない。
          *
          * これは研究利用への同意項目ではなく、同意するかどうかに関わらず起きる
          * ことなので、チェックボックスではなく説明として置いている。ここを
          * 書かずに通知だけ実装すると、「先生が本文を読むことはありません」の
          * すぐ下で、生徒が知らないうちに先生へ連絡が行くことになる。
          */}
        <h2 className="bl-h3">危険が疑われるとき</h2>
        <p className="bl-body">
          自分や誰かが危ないかもしれない、と読み取れる記述があったときは、
          すでにあなたの見守りに同意している先生に「確認が必要です」という連絡が自動で届きます。
          届くのは<strong>時刻と、確認が必要だということだけ</strong>で、
          あなたが書いた文章そのものは含まれません。
          これは研究への協力に同意するかどうかとは関係なく、安全のために行われます。
          誰にいつ連絡が行ったかは、あなた自身が「記録の閲覧」画面で確認できます。
        </p>
