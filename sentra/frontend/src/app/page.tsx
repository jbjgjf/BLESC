...
export default function TodayPage() {
  ...
  return demo ? <DemoTodayPage /> : <LiveTodayPage />;
}

function LiveTodayPage() {
  ...
  const recentCount = entries?.filter((entry) => {
    const entryDay = new Date(entry.created_at).toLocaleDateString("en-CA");
  ...
  return (
    <div className="bl-wrap bl-stack">
      <header className={`${styles.hello} bl-rise`}>
        ...
        <div>
          <h1 className="bl-h1">{greeting}</h1>
      ...
      <section className={`${styles.today} bl-rise`} data-done={submittedToday}>
        <div className={styles.todayIcon}>
          <Icon name={submittedToday ? "check_circle" : "edit_note"} size={30} fill={submittedToday} />
        </div>
