      ...
      {!isLoading && shares.length > 0 && (
        <section className="bl-card bl-rise" data-testid="summary-shares">
          <div className="bl-card-head">
            ...
            <h2 className="bl-h2">共有した支援サマリー</h2>
          </div>
            ...
            {shares.map((share) => (
              <li key={share.id}>
                <span>
                  ...
                  {share.counselor_user_id ? " · 指定したカウンセラー" : " · 所属カウンセラー"}
                  {" · "}{new Date(share.shared_at).toLocaleDateString("ja-JP")}
                  {" · "}{share.reflection_count}件の記録
                </span>
                {share.status === "active" ? (
                  <button
                    type="button"
                  ...
                  >
                    <Icon name="lock" size={15} />
                    {busyShareId === share.id ? "停止中…" : "共有を停止"}
                  </button>
                ) : (
                  <span className="bl-chip">停止済み</span>
                )}
              </li>
            ))}
          </ul>
